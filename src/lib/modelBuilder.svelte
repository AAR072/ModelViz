<script lang="ts">
  ...
  function useDefaults() {
    layers.update((current) => [
      { type: "dense", units: 32, activation: "relu", inputShape: "[1]" },
      { type: "dense", units: 32, activation: "relu", inputShape: "" },
    ]);
  }
  ...
  function buildModel() {
    ...
    userLayers.forEach((layer, index) => {
      ...
      if (layer.type === "dense") {
        ...
        model.add(tf.layers.dropout({ rate: parseFloat(layer.units) / 100 })); // Units as percentage
      } 
    });
...
<!-- Export Model Button -->
<button on:click={exportModel} class="boton-elegante" id="redElon">Start Training</button>
